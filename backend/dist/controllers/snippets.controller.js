...
export const getOrganizationSnippet = async (req, res) => {
    try {
        const organizationId = Number(req.params.organizationId);
        if (!organizationId) {
            return res
...
export const updateSnippets = async (req, res) => {
    try {
        const snippetId = Number(req.params.snippetId);
        if (!snippetId) {
            return res
        ...
        if (language !== undefined)
            updates.language = parsedLanguage ?? null;
...
export const getAllLanguages = async (_req, res) => {
    try {
...
export const createSnippetToCollection = async (req, res) => {
    try {
        const userId = req.user.userId;
...
export const getCollectionSnippets = async (req, res) => {
    try {
        const colId = Number(req.params.colId);
...
export const addExistingSnippetToCollection = async (req, res) => {
    try {
        const snippetId = Number(req.params.snippetId);
        const colId = Number(req.params.colId);
        if (!snippetId || !colId) {
            return res
                .status(400)
                .json({ success: false, message: "snippetId and colId are required" });
        }
        ...
        return res
            .status(200)
            .json({ success: true, message: "Added to collection", data: updated });
    }
    catch (error) {
...
export const getFavSnippets = async (req, res) => {
    try {
        const organizationId = Number(req.params.organizationId);
        ...
        return res.status(200).json({
            success: true,
            message: "favourite fetched success",
            data: favs,
        });
    }
    catch (error) {
