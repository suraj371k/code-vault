        ...
        <div className="space-y-3">
          {FAQ_ITEMS.map((item, idx) => (
            <details
              ...
              style={{
                animation: `section-reveal .5s ease ${idx * 80}ms both`,
              }}
            >
              <summary
                className="flex items-center justify-between font-semibold text-white text-sm
                                   select-none list-none group-hover:text-teal-300 transition-colors duration-200"
              >
                <span>{item.q}</span>
                <ChevronDown className="w-4 h-4 text-teal-400 shrink-0 chevron" />
              </summary>
